...
import FeedbackEntry, { FeedbackEntrySkeleton } from "../components/FeedbackEntry";
...
const RoadmapColumn = ({ status }: { status: "planned" | "inprogress" | "live" }) => {
	const { data: entries } = useSuspenseQuery({ queryKey: ["entries", { status }], queryFn: () => getEntriesHandler(status) });
	...
	return (
		<div className={styles.roadmapColumn}>
			<Title />
			{entries.map(entry => (
				<FeedbackEntry key={entry.id} entry={entry} link={true} roadmap={true} />
			))}
		</div>
	);
};
