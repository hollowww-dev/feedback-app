...
import { Category, Entry, EntryDetailed, NewEntry, Status } from "@/types";
import { findCategoryKey, findStatusKey } from "@/utils";
...
const EditPage = () => {
    ...
    onSuccess: (_data, content) => {
      if (content.status && content.status !== entry.status) {
        queryClient.setQueryData(
          ...
          (old: Entry[]) =>
            old && old.filter((oldEntry) => oldEntry.id !== entry.id)
        );
      }
      ...
      queryClient.setQueryData(
        ...
        (old: Entry[]) =>
          old &&
          old.map((entry) =>
            entry.id === params.id ? { ...entry, ...content } : entry
          )
      );
      queryClient.setQueryData(
        ["entries", { status: content.status || entry.status }],
  ...
  const { mutate: removeEntry, isPending: isPendingRemove } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.setQueryData(
        ["entries", { status: entry.status }],
        (old: Entry[]) => old && old.filter((entry) => entry.id !== params.id)
      );
      queryClient.invalidateQueries({ queryKey: ["stats"] });
      ...
      <form onSubmit={handleSubmit((data) => editEntry(data))}>
        ...
        <div className={styles.buttons}>
          <Button
            ...
            label="Save changes"
            variant="primary"
            disabled={isPendingRemove || isPendingEdit}
          />
          <Button
            type="button"
            label="Cancel"
